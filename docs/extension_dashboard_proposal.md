...
`new_mail_lookback_days` は未着眼件数だけに適用する。通常ワークフロー、専用タグ、待機状態は mailbox 全体を数える。未着眼は合計件数と「うち未読 / 既読」の内訳を1カードに表示し、Thunderbird作業ビューも未読・既読を合わせて同じ日数条件を適用する。

アーカイブ待ちもmailbox全体を対象とし、件数とThunderbird作業ビューを`一括処理（旧互換タグを含む）+ スターなし`で一致させる。
...
通常同期はExtensionダッシュボードの接続状態欄から開始できる。WIB接続不可時および同期実行中はボタンを無効にし、開始後は`/api/sync-status`を短い間隔で確認して実行中・完了を表示する。
...
Extensionダッシュボードから通常同期をバックグラウンドで開始する。すでに同期中の場合は二重起動せず、開始可否と現在の実行状態をJSONで返す。進捗と完了は既存の`GET /api/sync-status`で確認する。
