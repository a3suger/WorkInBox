...
`new_mail_lookback_days` は正式設計どおり未着眼・未読 / 未着眼・既読の2件数だけに適用する。通常ワークフロー、専用タグ、待機状態は mailbox 全体を数える。ダッシュボードの未着眼2カードから開くThunderbird作業ビューにも同じ日数条件を適用し、件数と一覧の対象期間を一致させる。
