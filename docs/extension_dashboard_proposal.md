...
`new_mail_lookback_days` は正式設計どおり未着眼・未読 / 未着眼・既読の2件数だけに適用する。通常ワークフロー、専用タグ、待機状態は mailbox 全体を数える。Thunderbird の未着眼作業ビューも現行どおり mailbox 全体を対象とするため、件数との差を画面に明記する。
