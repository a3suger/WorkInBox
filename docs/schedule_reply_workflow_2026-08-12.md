...
図では、一つの処理を通して同じメールを追跡できるよう、メールごとに participant を固定する。
...
sequenceDiagram
    ...
    alt 自分で対応
        ...
        Note over M1: WIBで完了操作
        Note over M1: スケジュール対応済み

    else 支援者へ依頼
        create participant M2 as 支援依頼メール
        M1->>M2: WIBで依頼メール生成・Thunderbirdで送信
        Note over M1: この時点では依頼済みを付けない
        Note over M2: 自分宛て/CcでINBOXへ到着

        Note over M2: 次回同期でTriageBoxが取り込む
        ...
        Note over M3: 回答必要 + ★
        Note over M1: スケジュール調整 + 依頼済み + ★ を維持

        alt M3が質問・追加確認
            Note over M3: 利用者が支援者へ回答
            Note over M1: スケジュール調整は未完了のまま
            Note over M1,M3: 支援者との往復を継続

        else M3が支援作業の完了報告
            Note over M3: 利用者が支援結果を確認

            alt 本来の送信者へ返信
                create participant M4 as 本来の送信者への返信メール
                M1->>M4: Thunderbirdで通常返信を送信
                Note over M1: スケジュール対応済み
                Note over M1: 一括処理 + ☆
                Note over M2: 対応待ち + 一括処理 + ☆
                Note over M3: 回答必要 + 一括処理 + ☆
                Note over M4: 返信待ち + ★
                Note over M4: 以後は通常の返信待ちワークフロー

            else 追加返信せず終了
                Note over M1: スケジュール対応済み + 一括処理 + ☆
                Note over M2: 対応待ち + 一括処理 + ☆
                Note over M3: 回答必要 + 一括処理 + ☆
...
M1 は、支援作業が完了したというだけでは自動完了しない。利用者が支援結果を確認し、本来の送信者への返信または案件終了を判断した時点で `スケジュール対応済み` となる。

#### M2: 支援依頼メール

M2 は TriageBox が同期で取り込んだ時点で `対応待ち` + スターとなる。

M3 が到着したら `対応待ち` は履歴として残し、`一括処理` を付けてスターを外す。これにより `WIB:対応待ち` から外れる。

#### M3: 支援者からの返信メール

M3 は、支援者からのメール一般ではなく、`対応待ち` の M2 に対する返信であることを relation と `In-Reply-To` / `References` で確認できたメールに限定する。

M3 には `スケジュール調整` を付けず、`回答必要` + スターを付ける。

質問か完了報告かは現時点では自動分類しない。利用者が内容を読んで次の行動を決める。

#### M4: 本来の送信者への返信メール

支援完了後に本来の送信者へ返信した場合は、その新しく送信した M4 が通常の `返信待ち` ワークフローの起点となる。

元メール M1 に `返信待ち` を付けない。

M4 をいつ・どの方法で `返信待ち` + スターとして取り込むかは、通常の Thunderbird 返信送信をどう捕捉するかの設計で別途決定する。
