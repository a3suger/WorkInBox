...
この図では、メール A / B / C を固定して追うことで、Triage によってどのメールの状態が変化するかを確認できる。

特に、支援者返信を受けた時点でも A のスケジュール調整は未完了であり、A のスターは維持する。一方 B は `対応待ち` を残したまま `一括処理` + スターなしとなり、現在作業の対象から外れる。
