...
したがって、支援依頼への返信を検出した場合は、最新返信メールを `回答必要` + スターとして現在の会話上の着眼点にする。
...
支援者から「支援作業が完了した」という内容のメールを受けた後、利用者が次のアクションを決める。
...
支援者から返信が来ると、支援依頼メールは `WIB:対応待ち` から外れて `一括処理` + スターなしとなり、最新返信メールが `WIB:回答必要` に現れる。
