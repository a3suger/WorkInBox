...
```mermaid
sequenceDiagram
    participant O as 起点メール
    participant R as 支援依頼メール
    participant S as 支援者
    participant T as TriageBox
    participant N as 支援者返信メール

    Note over O: スケジュール調整 + ★
    O->>R: WIBで支援依頼を作成・送信
    Note over R: 自分宛て/CcでINBOXへ到着
    T->>R: 同期で支援依頼を検出
    T->>O: 依頼済みを付与
    T->>R: 対応待ち + ★ を付与
    Note over O: スケジュール調整 + 依頼済み + ★
    Note over R: 対応待ち + ★

    S-->>R: 支援依頼への返信
    Note over N: 支援者から新着メールとして到着
    T->>T: In-Reply-To / References と relation を確認
    T->>R: 一括処理を付与・★解除
    Note over R: 対応待ち + 一括処理 + ☆
    T->>N: 回答必要 + ★ を付与
    Note over N: 回答必要 + ★
    Note over O: 未完了のため スケジュール調整 + 依頼済み + ★ を維持
```

この図では、支援者返信を受けた時点でも起点メールのスケジュール調整は未完了であること、また支援依頼メールの `対応待ち` タグ自体は削除せず、スター解除によって現在の `対応待ち` ビューから外すことを示している。
