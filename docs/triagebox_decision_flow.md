...
広告メール判定などの通常の TriageBox 処理を行い、その後 TrackingBox へ送るべきかどうかを判断する。
...
広告メール判定は TriageBox の通常責務として残す。

ただし、自分発メールの判定と Message-ID 関係の解析は、広告判定とは独立した機械的なメタデータ解析として扱う。

少なくとも自分が送信者のメールを広告として処理しないよう、From の自己判定を先に行う。

自分ではない送信者からの新規受信メールについては、広告判定を行ってから TrackingBox 入口判定へ進める。

返信関係が確認できるメールについては、追跡中メールへの返信を広告として誤除外しないよう、関係判定結果を広告判定にも利用する。
...
6. 関係がない通常受信メールについて広告判定・入口判定を行う。
7. 必要なスター・IMAP タグを更新する。
