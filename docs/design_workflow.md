...
- `見る・検討` から残す情報を Record として保存し、後から検索・利用できるようにする。
...
通常ワークフローは Thunderbird で対象メールを閲覧・処理する。

利用者が通常ワークフローを **終了** する場合は共通して次を行う。

- 元の通常ワークフロータグは残す。
- `一括処理` を付ける。
- スターを外す。

この共通終了操作は Thunderbird Extension から 1 操作で実行できる形を目指す。ボタン配置等は実装時に決める。
...
Thunderbird でメールを閲覧し、原則として次のいずれかへ進む。

1. **WIB に Record として保存する**
2. **通常ワークフローを終了する**

`まだ検討する` という独立した出口は設けない。将来の日時に再判断すべき仕事があるなら、必要に応じて締切として管理する。

Record として保存する場合は `一括処理` を付けず、`見る・検討` を残したままスターを外してアーカイブしてよい。

#### `注目`

自分から直ちに返信・処理する必要はないが、そのスレッドの今後の進展を継続して確認する必要がある状態を表す。

Thunderbird でメールを閲覧し、引き続き追う場合は active のまま維持する。もう追う必要がないと利用者が判断した場合は共通の通常ワークフロー終了操作を行う。

### 判定状態タグ

- `判定保留`

`判定保留` は **AI 判定済み・専用ワークフロー非該当・通常フローだけ未確定** を表す。単なる「未分類」ではない。
...
`見る・検討` のメールについて後から参照・検索・再利用する価値があると利用者が判断した場合、メールそのものを複製するのではなく、元メールへの参照を持つ **Record** を SQLite に作成する。
...
したがって `見る・検討` + スターなし + `一括処理` なしは、active な `見る・検討` ではなく Record として保存済みの参照状態になり得る。
...
Record の元メールは INBOX または Archive にある。
...
正式締切は `deadline -> source_message_id` で元メールの Message-ID を保持する。

締切自体が現在の作業対象であっても、元メールが active とは限らない。元メールがすでに通常ワークフロー終了済み・一括処理済み・アーカイブ済みである場合もあるため、締切画面から Message-ID を使って元メールを開けるようにする。

締切の元メールを開く場合も Record と同じ検索原則を用いる。
...
- スターが付いていない未読メール数
- スターが付いておらず、WIB の作業タグも付いていない既読メール数
...
- **未着眼・未読**: スターなし AND 未読
- **未着眼・既読**: スターなし AND 既読 AND WIB 作業タグなし
...
Record として保存済みの元メールは `見る・検討` タグを残してもスターがないため、active な `見る・検討` Quick Filter には出ない。一方、利用者が Archive フォルダを選択して `見る・検討` のタグだけで絞り込めば、保存済み元メールを Thunderbird 側から一覧できる。
...
3. 通常フローを `返信必要` / `見る・検討` / `注目` / `何もしなくてよい` から判定する。
...
締切は `deadline -> source_message_id` で起点メールと関連付ける。締切一覧・締切詳細から元メールを開けるようにし、元メールが INBOX にない場合は「元メール参照と Message-ID 検索」の検索原則を使う。
...
元メール M1 を起点に、利用者自身が予定確認・入力・調整を行う。
...
自分宛て/Cc の M2 を TriageBox が同期で確認した時点で、M1 に `依頼済み`、M2 に `対応待ち + ★` を付け、M1 と M2 の relation を SQLite に保存する。送信直後に元メールを先行変更しない。
...
`X-WorkInBox-Origin-Message-ID` は、専用ワークフローで M1 から標準 Reply ではない新規メール M2 を作成した場合に M1 と M2 を結ぶために利用する。
...
M2/M3 の支援者スレッドと M1/M4 の元メールスレッドは別の会話として扱う。
