...
`判定保留` は **AI が初期ワークフロー判定を実行済みであることを示す状態タグ** でもある。

スターが付いているにもかかわらず、専用ワークフロータグ、通常ワークフロータグ、`判定保留` のいずれも付いていないメールは、まだ TrackingBox の AI 初期判定を受けていない状態を表す。

一方、`判定保留` が付いているメールは、AI が判定を実行した結果、`締切あり` と `スケジュール調整` のどちらにも該当せず、かつ通常フローを `返信必要`、`見る・検討`、`何もしなくてよい` のいずれにも確定できなかったことを表す。したがって `判定保留` は「未判定」ではなく、「AI 判定済み・利用者判断待ち」の状態である。
...
スター付きメールについて、初期ワークフロー判定の状態はタグの有無で区別する。

- 専用ワークフロータグ、通常ワークフロータグ、`判定保留` のいずれも付いていないスター付きメールは、まだ AI 初期判定を受けていない。
- 専用ワークフロータグまたは通常ワークフロータグが付いているメールは、AI 初期判定によってワークフローが確定している。
- `判定保留` が付いているメールは、AI 初期判定は実行済みだが通常フローを確定できず、利用者判断を待っている。

TrackingBox は未判定のスター付きメールについて AI によるタグ付け支援を行い、その結果を IMAP タグへ自動反映する。
...
`判定保留` は、専用ワークフローに該当しないことを AI が先に判定したうえで、通常フローだけを確定できなかった状態である。つまり、AI 初期判定そのものは完了している。
