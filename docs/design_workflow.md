...
- `返信待ち` / `対応待ち` / `一括処理` は Thunderbird の WIB 専用 Quick Filter 作業ビューで確認・整理する。
...
1. `判定保留` は WIB の TrackingBox `判定保留` 一覧で利用者が再分類する。
...
6. `返信待ち` かつ一定期間経過したメールは `WIB:返信待ち` ビュー等から必要に応じて催促する。
