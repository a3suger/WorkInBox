...
`依頼済み` が付いた起点メールは支援者への依頼作成段階を終えているため、未依頼の `スケジュール調整` 件数とWIB Webのスケジュール調整一覧から除外する。支援中の状況は、依頼メール側の `対応待ち` / `対応あり` で確認する。
...
`対応待ち`としてSQLiteに保存されたM2については、直近期間の`In-Reply-To` / `References`を対象に返信M3を既読・未読に関係なく検索する。M3からM2の状態を確認するときも、SQLiteの保存済みUIDを優先して直接取得し、受信箱全体のMessage-ID検索を避ける。
...
- Thunderbird Extensionから、自分宛てのRecord登録依頼メールを作成する。
- 件名を必須のRecordタイトル、本文を任意のnoteとする。
- 特殊ヘッダーにRecord登録種別、元メールのMessage-ID、一意な依頼IDを保持する。
- 送信成功後、元メールを`一括処理 + スターあり`の自動処理待ちにする。送信中止・失敗時は変更しない。
- TriageBoxが依頼IDによる重複防止を行ってRecordを作成し、TrackingBoxが元メールのAI要約を`summary`へ保存する。
- 一連の処理が成功した後、元メールと登録依頼メールを`一括処理 + スターなし`にする。失敗時はスターを残して再実行可能にする。
- 元の通常ワークフロータグは残す。
- Recordの一覧と詳細はWIB Webで閲覧し、元メールへの導線を持つ。
...
- `title` は登録依頼メールの件名として利用者が入力する。
- `summary` は AI が自動生成する。
- `note` は登録依頼メールの本文として利用者が任意に入力する。
...
`対応あり + ★`のM3を表示している場合、ExtensionのWIB操作メニューは通常フローとほかの専用フローを隠し、次の3操作だけを表示する。

- `追加の質問を送る`: M3への返信M5を作成する。送信時にM3を変更せず、通常同期のrelation処理がM3を`一括処理 + ☆`、M5を`対応待ち + ★`として着目点を移す。
- `お礼を送って終了`: M3への返信M5を作成する。送信成功後にExtensionがM3へ`一括処理`だけを付け、通常同期のrelation処理がM3のスターを外し、M5も`一括処理 + ☆`として終了する。
- `返信せず終了`: ExtensionがM3を直ちに`一括処理 + ☆`とする。

追加質問とお礼の作成メールには操作種別を示すWIBヘッダーを付ける。通常はM3の`一括処理 + ★`が「お礼送信済み」の永続的な判定材料となるが、送信済みメールの到着とIMAPキーワード反映が前後した場合は操作種別ヘッダーを補助情報として使用する。作成中止・送信失敗時はM3を変更しない。
