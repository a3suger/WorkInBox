...
- 追加実装Issue: #26・#27 Closed、#28〜#30 Open
- 現在位置: #27 の実装、自動テスト、Actions、Thunderbird実機確認を完了してClose。次は #28
...
実機確認 #17・#23 と追加実装 #26・#27 は完了しClosed。実機確認 #18〜#22・#24・#25と、追加実装 #28〜#30が残っている。
...
#27 は完了。次は #28、続いて #29を進める。#30はThunderbird標準の`mid:`処理で表示先を指定できるか調査してから設計する。
...
現時点で #6〜#17・#23・#26・#27 はCloseしているが、#18〜#22・#24・#25・#28〜#30がOpenのため、
#5 は Open のまま維持されている。
...
1. この中断記録をcommitしてGitHubへ反映する。
2. #28 のIssue、`docs/design.md`、締切支援の「このメールには締切なしとして終了」の実装を確認する。
3. スケジュール調整支援へ「スケジュール調整は不要」を追加する設計・実装へ進む。
4. #28完了後は #29、必要な調査時期に #30へ進む。
