...
メニュー構成自体は利用者が理解できることを確認した。専用フローは、開始・再開時のタグとスター、対象Message-IDを指定したWIB画面、`締切なし`・`スケジュール調整なし`後の終了規則まで実装・実機確認済み。通常フローでは、現在付いているタグに対応する項目（例: `回答必要`が付いているときの「回答必要にする」）を選択不可にする案を検討中。複数の通常タグを許すか、分類変更時に以前の通常タグを外すか、通常タグを履歴として残すかは未決定である。
...
WIB操作メニューは専用フロー部分まで実装済み。通常フロー部分の動作だけが設計判断待ちである。

## 次に行うこと

1. 決定済みの「追加の質問を送る」「お礼を送って終了」「返信せず終了」をWIB操作メニュー、Extension、TrackingBoxへ実装する。
2. WIB操作メニューの通常フローで、複数タグを許すか、分類変更時に旧タグを外すか、履歴をどう残すかを議論する。
3. 現在付いている通常タグの項目を選択不可にする表示を含め、通常フロー各項目の動作を確定して実装する。
4. 残る実機確認 #18〜#22・#24・#25 と追加実装 #28〜#30を実運用に合わせて進める。

## 設計判断待ち

- 通常フロータグを排他的な現在分類として扱うか、複数を履歴として共存させるか。
実機確認の結果、新しい設計判断が必要になった時点で停止して利用者へ確認する。今回の中断は設計判断待ちによる停止ではなく、利用者が本日の作業を終了したための通常中断である。
