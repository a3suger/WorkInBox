...
ThunderbirdのToDo詳細に表示される「関連リンク」から、元メールを素早く直接表示できることを実機確認し、利用者の指示により作業を一旦中断する。
...
VTODOは読み取り専用のためThunderbird上で締切を修正できない。ToDo詳細から該当するWIBの締切詳細ページへ移動し、WIB側でタイトル・期限・メモを修正して、再生成されたVTODOへ反映できる導線を追加する案を次回検討する。SQLiteを正本とする現行設計は維持し、Thunderbirdからの双方向編集は行わない。元メールを直接開く `mid:` の「関連リンク」と、WIB締切詳細を開くリンクの表示場所を区別する必要がある。
...
6. commit `94b0aec` までのGitHub Actions結果を確認する。
7. 通常運用中に見つかった違和感・エラー・確認済み項目を利用者へ確認する。
8. 最初にWIB内の元メール導線へ `mid:` を展開する範囲とフォールバックを確認する。実装しない場合は、`docs/manual_test_runbook.md` に従い #22の未確認項目または #24 + #25 の継続確認から再開する。
