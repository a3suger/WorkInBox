...
実運用中、Thunderbirdで締切を確認している画面から元メールへ戻れない事象があった。締切情報に対応するMessage-IDをWIB側で取得し、Thunderbird Bridgeを通して元メールを表示する導線が必要。再開時に、既存の締切一覧・詳細にある元メール導線の不具合なのか、Thunderbird側の別画面に新しい導線が必要なのかを利用者の操作手順と照合して切り分ける。
