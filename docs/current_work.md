...
作業中（スケジュール支援依頼の本文内転送`0.3.5`実装・自動テスト完了、commit・Actions・実機確認待ち）
...
- 現在位置: 支援者への依頼メールを別スレッドのまま元メールの本文内転送を含めるよう、Extension `0.3.5`として実装。自動テスト143件成功
...
1. Extension `0.3.5`の本文内転送対応をcommitしてGitHubへpushする。
2. GitHub Actionsの成功を確認する。
3. noteでExtensionを再読み込みし、スケジュール調整から支援依頼メールを作成する。
4. M2がM1へのReplyではなく別スレッドで、本文にM1が転送され、宛先・Cc・件名・本文冒頭が正しいことを確認する。
5. 送信後にM1へ`依頼済み`が付き、通常同期後にself-CcのM2が`対応待ち + ★`になることを確認する。
6. #31の未着眼件数差と端末間表示差は別途修正・確認する。
