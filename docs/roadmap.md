...
スケジュール調整支援を手動タグ操作なしで E2E 確認できるよう、TriageBox のうちヘッダと既存状態だけで判定できる決定論的な基盤も v0.2 に含める。
...
### 8.6. Thunderbird Quick Filter 作業ビュー【未実装】

WorkInBox の一覧やダッシュボードから、Thunderbird の通常のメール一覧へ戻って作業できる導線を用意する。

主な機能:

- 既存の Thunderbird メールタブを INBOX へフォーカスする。
- Quick Filter Bar を有効にする。
- WorkInBox の作業タグとスターを AND 条件にしたプリセットを適用する。
- `回答必要を見る` などの WIB UI から Thunderbird の作業ビューへ直接遷移する。
...
Thunderbird の Quick Filter に「返信済み」条件がなくても、WIB の同期で現在注目対象から外れたメールのスターを外すことで、`タグ AND スター付き` の作業ビューから自然に除外する。

検索フォルダーの自動生成は v0.2 の必須条件にせず、まず標準 MailExtension API で実現できる Quick Filter 作業ビューを優先する。
...
- 利用者自身で対応するフロー
- 支援者への依頼メール作成支援
- WIB 作成支援メールへの `X-WorkInBox-Origin-Message-ID` 付与
- 元メールへの `依頼済み`
- TriageBox 基盤による依頼メールへのスター + `対応待ち`
- 支援者からの返信検出
- 回答・反映完了後の `スケジュール対応済み`

### 9c. TriageBox 決定論的基盤【未実装】

v0.2 のワークフローを手動タグ付けなしで E2E 確認するため、AI を使わずヘッダと既存状態だけで決定できる TriageBox の基盤を実装する。

主な機能:

- 設定済み自己アドレスによる `From` 自己判定
- `X-WorkInBox-Origin-Message-ID` による WIB 作成支援メールの識別と元メール関連付け
- WIB のスケジュール支援依頼メールへのスター + `対応待ち`
- `In-Reply-To` / `References` の解析
- `返信待ち` / `対応待ち` メールへの返信検出
- 元メールから新着返信メールへの着目点移動
- スケジュール支援回答から元メールへ戻るための関連情報保持
- 回答済み・処理済みメールを現在注目対象から外すためのスター状態更新

AI 広告判定、広告メールの分類、催促対象判定等はこの段階には含めない。

詳細は `docs/triagebox_decision_flow.md` を参照する。
...
- WIB の作業一覧から Thunderbird の `タグ AND スター付き` Quick Filter 作業ビューへ移動できる
- 締切を SQLite に正式登録できる
- Thunderbird で締切を read-only ICS として確認できる
- スケジュール調整の支援依頼から `対応待ち`、返信到着、確認・反映までを手動タグ操作なしで確認できる
- 特殊処理後に次の作業へ遷移できる
...
## v0.3 以後 — TriageBox 拡張

TriageBox の決定論的な関係判定と v0.2 ワークフローに必要な待機状態遷移は v0.2 へ前倒しする。

v0.3 以後では、本文判定や時間経過を必要とする拡張機能を追加する。

詳細は `docs/triagebox_decision_flow.md` を参照する。

主な候補:

- 送信者・件名・メールゲートウェイ判定等による決定論的な事前分類
- 既存ThunderbirdメッセージフィルターからWIB管理ルールへの移行
- `新規受信メール` のみに対する AI 広告判定
- 広告メールの処理
- TrackingBox 入口判定の自動化
- 通常の自分発メールを `返信待ち` とする詳細判定
- 催促対象の判定
- 待機日数・期限に基づく通知
- Sent フォルダを含む監視範囲の拡張
- 参照先 Message-ID がローカルにない場合の追加探索
