...
Extension `0.3.25`では、締切・スケジュール調整の専用ワークフローだけを持つメールでも、専用処理の完了後に通常終了またはRecord保存終了を選べるようにしました。専用処理が未完了のメールは誤って終了できません。

Extension `0.3.26`では、専用フローの開始・継続・「なし」をメールのタグ状態に応じて表示します。未完了の専用フローがある間は終了操作を隠し、専用タグをすべて外して他のWIBタグがない場合は`判定保留`へ移します。

Extension `0.3.27`では、`注目`タグが付いたメールでRecord保存終了を選べないようにしました。通常終了は引き続き選択できます。

Extension `0.3.28`では、手動でタグを変更した直後でもメール詳細メニューが最新のタグ状態を再取得して判定するようにしました。

Extension `0.3.29`では、最新タグの再取得が遅い場合もメール詳細メニューを長時間待たせず、取得済みのメール状態で表示するようにしました。

Extension `0.3.30`では、メール詳細メニュー表示時の再検索を行わず、ポップアップを即時表示するようにしました。タグの再確認は操作実行時に行います。

Extension `0.3.31`では、未着目ダッシュボード件数とThunderbird作業ビューの期間条件を同じローカル日付境界で判定するようにしました。

Extension `0.3.32`では、設定・ツール画面から対象アカウント / フォルダ、期間、検索条件、各クエリの取得件数を診断JSONとして確認・コピー・保存できるようにしました。desktop と note の結果比較に使用します。

Extension `0.3.33`では、診断JSONに各取得メールのタグ・既読・スター状態を含め、設定・ツール画面からクリップボードへコピーできるようにしました。

Extension `0.3.34`では、旧キーのタグと同じ表示名の新タグを登録する際、既存タグを再利用して重複エラーにならないようにしました。

Extension `0.3.35`では、旧キー `wib-batch` を再利用している場合も、設定画面のWIBタグ確認数を正しく13/13と表示します。

Extension `0.3.42`では、ダッシュボードからスター付きメール（Active）をThunderbird上で一覧表示できるようにしました。
