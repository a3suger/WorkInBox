...
class SupportRequestContractTest(unittest.TestCase):
    def test_support_request_is_inline_forward_with_origin_header(self) -> None:
        background = (
        ...
        self.assertNotIn("messenger.compose.beginNew()", background)
    ...
    def test_web_bridge_no_longer_sends_reply_or_forward_mode(self) -> None:
        ...
        self.assertIn("別スレッドの依頼メール", template)
        self.assertIn("元メールを本文内転送", template)
