...
class ThunderbirdWorkViewContractTest(unittest.TestCase):
    ...
    def test_message_display_action_opens_preview_menu(self) -> None:
        ...
        self.assertIn("openDedicatedWorkflow(request.kind, request.messageId)", background)
        self.assertIn("dismissDedicatedWorkflow(request.kind, request.messageId)", background)
        ...
        self.assertIn("prepareDedicatedWorkflow(request.kind, request.messageId)", background)
    ...
    def test_extension_dashboard_is_registered_and_opened_in_a_reused_tab(self) -> None:
        ...
        self.assertEqual(manifest["version"], "0.3.18")
