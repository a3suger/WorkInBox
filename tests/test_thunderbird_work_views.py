...
class ThunderbirdWorkViewContractTest(unittest.TestCase):
    ...
    def test_message_display_action_opens_contextual_workflow_menu(self) -> None:
        ...
        self.assertIn("OPEN_WORKFLOW_TAGS.has(tag) || DEDICATED_DONE_TAGS.has(tag)", background)
        self.assertIn('"wib-pending"', background)
        self.assertIn("tags: pendingTags", background)
        ...
        self.assertIn("refreshDisplayedMessageTags", background)
    ...
    def test_extension_dashboard_is_registered_and_opened_in_a_reused_tab(self) -> None:
        ...
        self.assertEqual(manifest["version"], "0.3.38")
